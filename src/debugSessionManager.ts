        ...
        return {
            ...
            onDidSendMessage: async (message) => {
                ...
                if (message.type === "response" && message.command === "launch") {
                    if (launchPromiseResolver) {
                        launchPromiseResolver();
                        launchPromiseResolver = null;
                    }
                    if (message.body && message.body.spawnerPid) {
    ...
    private async updateProcessesFromAdapter(session: vscode.DebugSession): Promise<void> {
        ...
        try {
            ...
            if (response && Array.isArray(response.processes)) {
                const processes: DebugProcess[] = response.processes.map((process) => ({
                    pid: process.pid,
                    name: process.name
                }));

                const currentPid = response.currentProcess ?? 0;
                const activeProcess = processes.find(process => process.pid === currentPid) || processes[0];
